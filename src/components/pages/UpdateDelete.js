...
function UpdateDelete() {
    ...
    const fetchData = async () => {
        const { data } = await axios.get("http://139.84.230.196/api/displayAll");
        setMinerals(data);

    }

    async function deleteOperation(n) {
        let result = await fetch("http://139.84.230.196/api/delete/" + n, {
            method: 'DELETE'
        });
                    ...
                    <tbody>
                        {
                            minerals.map((mineral) =>
                                <tr>
                                    ...
                                    <td>
                                        <img className='min-pic' src={"http://139.84.230.196/" + mineral.image_path} alt="Mineral pic" />
